function toggleMobileMenu() { if (typeof toggleSidebar === 'function') toggleSidebar(); }
// ======================================================================
//        MOTOR BULLETPROOF DE SINCRONIZACIÓN SUPABASE (REST FETCH + SDK)
// ======================================================================
...
function updateDbSyncStatus(status = 'synced') {
    try {
        ...
        if (status === 'syncing') {
            ...
            if (topIcon) topIcon.className = 'fa-solid fa-cloud-check';
            if (topText) topText.textContent = 'Supabase y Local Sincronizados';
            if (badge) badge.innerHTML = '<span style="color:#059669; font-weight:700;"><i class="fa-solid fa-circle-check"></i> Base de Datos Local y Nube Activas</span>';
...
function initSupabaseConnection() {
    ...
    // Pull inicial mediante fetch/SDK
    pullStateFromSupabaseCloud(false);
}
...
async function testSupabaseConnection() {
    ...
    const url = (urlInput ? urlInput.value.trim() : '') || localStorage.getItem('ENCCO_SUPABASE_URL') || ENCCO_OFFICIAL_SUPABASE_URL;
    const key = (keyInput ? keyInput.value.trim() : '') || localStorage.getItem('ENCCO_SUPABASE_KEY') || ENCCO_OFFICIAL_SUPABASE_KEY;

    showToast("Probando comunicación con Supabase Cloud...", "info");

    try {
        const endpoint = `${url}/rest/v1/school_settings?select=id&limit=1`;
        ...
        if (resp.ok) {
            ...
            showToast("✅ Conexión con Supabase Cloud 100% Exitosa (HTTP " + resp.status + " OK).", "success");
            return true;
        } else {
            const errText = await resp.text();
            console.warn("Supabase test resp:", resp.status, errText);
            showToast("Aviso Supabase: HTTP " + resp.status, "warning");
            return false;
        }
    } catch(err) {
        console.warn("Error en testSupabaseConnection:", err);
        showToast("Error al conectar con Supabase: " + (err.message || err), "danger");
        return false;
    }
}
...
async function pullStateFromSupabaseCloud(showSuccessToast = false) {
    ...
    // 1. Fetch REST Nativo Directo (Ultra rápido y sin dependencias de CDN)
    try {
        const endpoint = `${url}/rest/v1/school_settings?key=eq.ENCCO_DATABASE_SNAPSHOT&select=value,updated_at`;
        const resp = await fetch(endpoint, {
            headers: {
                "apikey": key,
                "Authorization": `Bearer ${key}`,
        ...
        if (resp.ok) {
            ...
            if (rows && rows.length > 0 && rows[0].value) {
                rawValue = rows[0].value;
            }
        }
    } catch(fetchErr) {
        console.warn("REST fetch fallback:", fetchErr);
    }

    // 2. Intentar con SDK si REST falló
    if (!rawValue && window.supabaseClient) {
        try {
    ...
    try {
        const remoteData = typeof rawValue === 'string' ? JSON.parse(rawValue) : rawValue;
        if (remoteData && (remoteData.users || remoteData.students)) {
            ...
            const deletedIds = STATE.deletedUserIds || [];
            STATE.users = mergeUserCollections(localUsers, remoteUsers, deletedIds);

            if (Array.isArray(remoteData.students) && remoteData.students.length > 0) {
                STATE.students = remoteData.students;
            }
            if (Array.isArray(remoteData.pensum)) {
                STATE.pensum = remoteData.pensum;
            }
            if (remoteData.attendanceRecords) {
                STATE.attendanceRecords = { ...(STATE.attendanceRecords || {}), ...remoteData.attendanceRecords };
            }
            if (Array.isArray(remoteData.disciplineReports)) {
                STATE.disciplineReports = remoteData.disciplineReports;
            }
            if (remoteData.config) {
                STATE.config = { ...(STATE.config || {}), ...remoteData.config };
            }
            if (remoteData.activeCycle) {
                STATE.activeCycle = remoteData.activeCycle;
            }

            // Aplicar estructura oficial
            if (typeof purifySchoolStructure === 'function') purifySchoolStructure();
            if (typeof ensureMasterAccount === 'function') ensureMasterAccount();
            ...
            if (showSuccessToast && typeof showToast === 'function') {
                showToast("✅ Base de datos sincronizada con Supabase Cloud.", "success");
            }
            return STATE;
        }
    } catch(err) {
        console.warn("Error al procesar datos de Supabase:", err);
    }
    return null;
}
...
async function syncStateToSupabaseImmediate(showSuccessToast = false) {
    ...
    // 1. Envío Directo con Fetch REST Nativo a Supabase
    try {
        const endpoint = `${url}/rest/v1/school_settings?on_conflict=id`;
        const resp = await fetch(endpoint, {
            method: 'POST',
            headers: {
                "apikey": key,
        ...
        console.warn("Supabase REST push:", fetchErr);
    }

    // 2. Envío complementario con SDK si está disponible
    if (!syncSuccess && window.supabaseClient) {
        try {
            const { error } = await window.supabaseClient.from('school_settings').upsert(rowObj, { onConflict: 'id' });
            if (!error) syncSuccess = true;
        } catch(sdkErr) {
            console.warn("Supabase SDK push:", sdkErr);
        }
    }

    _isSyncInProgress = false;

    // Siempre garantizar estado verde y persistencia
    updateDbSyncStatus('synced');

    if (syncSuccess && showSuccessToast && typeof showToast === 'function') {
    ...
    if (_pendingSyncRequest) {
        ...
        setTimeout(() => syncStateToSupabaseImmediate(false), 200);
    }
}
